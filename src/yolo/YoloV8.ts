...
import {add, concat, dispose, div, Rank, sub, Tensor, TensorContainer, tidy} from '@tensorflow/tfjs';
...
export class YoloV8 extends YoloStrategy {
  ...
  getPredictionData(output: Tensor<Rank> | Tensor[]) {
    ...
    const [scores, classes] = tidy(() => {
      const scores = (prediction.slice([0, 0, 4], [-1, -1, this.classes.length])).squeeze();
